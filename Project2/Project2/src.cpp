...
void Laplace_Conv(BYTE* Img, BYTE* Out, int W, int H) // Laplace 마스크
{
	double Kernel[3][3] = { -1.0, -1.0, -1.0,
							-1.0, 8.0, -1.0,
							-1.0, -1.0, -1.0 };
...
void Laplace_Conv_DC(BYTE* Img, BYTE* Out, int W, int H) // Laplace 마스크 DC
{
	double Kernel[3][3] = { -1.0, -1.0, -1.0,
							-1.0, 9.0, -1.0,
							-1.0, -1.0, -1.0 };
...
void DrawRectOutlineColor(BYTE* Img, int W, int H, int LU_X, int LU_Y, int RD_X, int RD_Y, BYTE R, BYTE G, BYTE B)
{
	for (int i = LU_X; i < RD_X; i++)
	{
		Img[LU_Y * W * 3 + i * 3 + 2] = R;
...
void Obtain2DBoundingBoxColor(BYTE* Image, int W, int H, int* LU_X, int* LU_Y, int* RD_X, int* RD_Y)
{
	int flag = 0;
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if (Image[i * W * 3 + j * 3 + 2] > 0 ||
				Image[i * W * 3 + j * 3 + 1] > 0 ||
				Image[i * W * 3 + j * 3    ] > 0
				)
			{
...
int main()
{
	...
	*/
	...
	//Rotation(Image, Output, W, H, 60); //원점 중심 회전
	...
	//Obtain2DBoundingBox(Output, W, H, &LU_X, &LU_Y, &RD_X, &RD_Y); // 이진영상의 외접직사각형 좌표 추출

	//DrawCrossLine(Image, W, H, Cx, Cy);
	//DrawRectOutline(Image, W, H, LU_X, LU_Y, RD_X, RD_Y);
	...
	m_BlobColoring(temp3, H, W); // 라벨링 결과 가장 큰 영역만 남김
	int LU_X, LU_Y, RD_X, RD_Y;
	Obtain2DBoundingBox(temp3, W, H, &LU_X, &LU_Y, &RD_X, &RD_Y);
	DrawColorRectOutline(Image, W, H, LU_X, LU_Y, RD_X, RD_Y, 255, 0, 0);
	
	free(temp3);*/

	/*int LU_X, LU_Y, RD_X, RD_Y;
	Obtain2DBoundingBoxColor(Output, W, H, &LU_X, &LU_Y, &RD_X, &RD_Y);
	DrawRectOutlineColor(Output, W, H, LU_X, LU_Y, RD_X, RD_Y, 255, 0, 0);*/
	...
	SaveBMPFile(hf, hInfo, hRGB, Image, hInfo.biWidth, hInfo.biHeight, "result.bmp");
